...
bool is_safe_impl(PosList* levels, gint64 skip_index) {
    if (skip_index >= 0)
...
bool is_safe(PosList* levels, bool use_problem_dampener) {
    gint64 level_count = g_list_length(levels);
    if (!use_problem_dampener)
        return is_safe_impl(levels, -1);
    for (int ll = 0; ll < level_count; ll++) {
        PosList* levels_copy = g_list_copy(levels);
        PosList* to_free = ll == 0 ? levels_copy->next : levels_copy;
