...
gint64 get_tokens_needed_to_win_or_zero(Int64List** ints, gint64 goal_error) {
    ...
    gint64 a = (gint64)round((y_goal - b_xy_factor * x_goal) / ay_without_x);
    double a_xy_factor = ay / ax;
    double by_without_x = by - (a_xy_factor * bx);
    gint64 b = (gint64)round((y_goal - a_xy_factor * x_goal) / by_without_x);
...
gint64 get_tokens_needed_to_win_all_possible(char* claw_machines, gint64 goal_error) {
    ...
    Int64List* ints = int_list_parse(claw_machines);
    while (ints)
